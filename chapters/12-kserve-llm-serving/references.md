...
| vLLM Online Serving | https://docs.vllm.ai/en/latest/serving/online_serving/ | vLLM이 제공하는 OpenAI-compatible API 구조. KServe runtime 내부 이해에 도움 |

## 업데이트 가능성이 큰 정보

- KServe version에 따라 CRD schema, runtime 이름, gateway 방식이 바뀔 수 있다.
- Hugging Face runtime image는 CUDA version, vLLM version, supported architecture가 바뀔 수 있다.
- vLLM engine argument 이름과 기본값은 release에 따라 추가/변경될 수 있다.
- Qwen model card, license, file structure는 Hugging Face model page 기준으로 다시 확인해야 한다.
- autoscaling behavior는 KServe deployment mode와 autoscaler 구현에 따라 다르게 보일 수 있다.
- gateway 호출 방식은 Istio, Knative, Gateway API, cluster 환경마다 달라질 수 있다.
