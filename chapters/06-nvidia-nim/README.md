...
이 문서는 2026-06-29 기준 공식 문서를 바탕으로 작성했다.  
실습 전 [references.md](references.md)의 NVIDIA 공식 문서와 NGC catalog를 다시 확인한다.
...
1. [../../GLOSSARY.md](../../GLOSSARY.md)에서 NIM, NGC, registry, API key 관련 용어를 확인한다.
2. 아래 핵심 개념 요약을 읽는다.
3. [references.md](references.md)에서 공식 문서와 NGC catalog 확인 위치를 본다.
4. [scripts/01_check_env.sh](scripts/01_check_env.sh)로 Docker/GPU/client 환경을 확인한다.
5. NGC 계정과 API key를 준비한다.
6. [scripts/02_ngc_login.sh](scripts/02_ngc_login.sh)로 NGC registry login을 수행한다.
7. [scripts/03_pull_nim_image.sh](scripts/03_pull_nim_image.sh)로 NIM image를 pull한다.
8. [scripts/04_run_nim_container.sh](scripts/04_run_nim_container.sh)로 NIM container를 실행한다.
9. [scripts/05_curl_models.sh](scripts/05_curl_models.sh)와 [scripts/06_curl_chat.sh](scripts/06_curl_chat.sh)로 API를 호출한다.
10. [client/07_openai_client.py](client/07_openai_client.py)로 OpenAI SDK 호출을 확인한다.
11. [scripts/08_compare_latency.sh](scripts/08_compare_latency.sh)로 같은 prompt를 여러 번 호출해 latency를 비교한다.
12. [scripts/09_collect_runtime_info.sh](scripts/09_collect_runtime_info.sh)로 GPU/container/log 상태를 기록한다.
13. [scripts/10_stop_container.sh](scripts/10_stop_container.sh)로 container를 종료한다.
14. 결과를 [templates/lab-notes.md](templates/lab-notes.md)에 정리한다.
