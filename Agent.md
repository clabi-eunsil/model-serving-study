...
- 아키텍처, request flow, data flow, control plane/data plane, storage/cache 구조처럼 글만으로 이해하기 어려운 설명이 나오면 다이어그램이 도움이 될지 먼저 사용자에게 제안한다.
- 사용자가 다이어그램 생성을 원하면 공식 문서를 기준으로 구성요소와 흐름을 확인하고, 근거 URL을 함께 기록한다.
- 다이어그램은 가능하면 `assets/diagrams/` 아래 SVG로 만든다.
- 박스 안 텍스트와 선이 겹치지 않게 배치한다.
- 선은 컴포넌트를 가로지르지 않고 여백을 따라 꺾은선으로 그린다.
- 선 위에는 긴 설명을 쓰지 않고 번호/알파벳만 표시한다.
- 상세 설명은 그림 아래에 번호별 흐름 표로 정리한다.
- 실선은 주요 흐름, 점선은 보조 흐름으로 구분한다.
- 처음 보는 사람도 이해할 수 있도록 컴포넌트 역할과 데이터 흐름을 한국어로 설명한다.
- SVG/XML 파싱 검증을 수행하고, 잘린 글자나 겹치는 요소가 없는지 확인한다.
