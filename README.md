...
- [x] KServe가 제공하는 추상화 이해
- [x] `InferenceService` 리소스 구조 학습
- [x] predictor, transformer, explainer 개념 이해
- [x] built-in runtime과 custom runtime 차이 이해
- [x] Knative, Istio, Gateway와의 관계 정리
- [x] autoscaling과 scale-to-zero 개념 이해
- [x] 실습: KServe 설치 상태 확인
- [x] 실습: sklearn 또는 간단한 torch model 예제 배포
- [x] 실습: `InferenceService`로 endpoint 호출
- [x] 실습: autoscaling 동작 확인
