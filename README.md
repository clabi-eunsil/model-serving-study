...
- [x] 1. 모델 서빙 기본 개념: [chapters/01-basic-concepts/README.md](chapters/01-basic-concepts/README.md)
- [x] 2. 로컬 FastAPI 모델 서빙: [chapters/02-fastapi-serving/README.md](chapters/02-fastapi-serving/README.md)
- [x] 3. Docker 기반 모델 서빙: [chapters/03-docker-serving/README.md](chapters/03-docker-serving/README.md)
- [x] 4. vLLM 입문: [chapters/04-vllm-intro/README.md](chapters/04-vllm-intro/README.md)
...
- [ ] concurrency, latency, throughput 관계 이해
- [ ] prompt length, output token length, KV cache, GPU memory 관계 이해
- [ ] `--max-model-len`, `--gpu-memory-utilization`, `--max-num-seqs`, `--max-num-batched-tokens` 역할 이해
- [ ] prefix caching, chunked prefill, quantization, tensor parallelism 개념 정리
- [ ] 실습: vLLM server를 튜닝 가능한 옵션으로 실행
- [ ] 실습: concurrency별 latency/throughput 측정
- [ ] 실습: prompt 길이와 output token 수를 바꿔 benchmark
- [ ] 실습: streaming TTFT와 total latency 측정
- [ ] 실습: GPU memory와 server log 기록
- [ ] 실습 마무리: benchmark 결과 CSV 정리, server 종료, `.venv` deactivate
...
- [ ] NIM의 목적과 사용 사례 이해
- [ ] NGC catalog, container registry, API key 개념 이해
- [ ] NIM과 직접 vLLM 운영 방식 비교
- [ ] NIM의 라이선스, 지원 모델, 배포 제약 확인
- [ ] OpenAI-compatible API 제공 방식 이해
- [ ] 실습: NIM container pull
- [ ] 실습: NIM container 실행
- [ ] 실습: NIM endpoint에 chat completions 요청
- [ ] 실습: vLLM과 동일 prompt로 latency 비교

## 7. 성능 테스트 방법론

- [ ] TTFT, TTFB, TTFP 차이 정리
- [ ] end-to-end latency와 inter-token latency 차이 이해
- [ ] p50, p90, p95, p99 latency 해석 방법 학습
- [ ] requests/sec와 tokens/sec 차이 이해
- [ ] throughput과 latency trade-off 이해
- [ ] `wrk`, `hey`, `k6`, `locust` 용도 비교
- [ ] vLLM benchmark script 사용법 학습
- [ ] 실습: custom Python async benchmark 작성
- [ ] 실습: concurrency별 latency 측정
- [ ] 실습: input/output token 길이별 결과 비교
- [ ] 실습: benchmark 결과를 표로 정리
...
- [ ] Langfuse가 해결하는 문제 이해
- [ ] trace, span, generation 개념 학습
- [ ] prompt, completion, latency, token usage 추적 방식 이해
...
- [ ] Deployment, Service, Ingress로 모델 서버 배포하는 흐름 이해
- [ ] GPU node scheduling 이해
- [ ] `nvidia.com/gpu` resource request 사용법 학습
- [ ] nodeSelector, taints, tolerations 개념 정리
- [ ] PVC로 model cache 관리하는 방법 학습
- [ ] readiness probe와 liveness probe 설계
- [ ] 실습: FastAPI 모델 서버를 Kubernetes Deployment로 배포
- [ ] 실습: GPU resource request 설정
- [ ] 실습: Service로 endpoint 노출
- [ ] 실습: Ingress 또는 port-forward로 호출
- [ ] 실습: rolling update 시 모델 서버 동작 확인
