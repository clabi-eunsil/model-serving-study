...
| Hugging Face Runtime | KServe Hugging Face 서빙 런타임 | Serving Runtime | kserve, huggingface, vllm | [12](chapters/12-kserve-llm-serving/README.md) | KServe에서 Hugging Face model을 serving하기 위한 runtime. generative inference에서는 vLLM backend를 사용해 LLM text generation을 처리한다. |
| `hf://` URI | Hugging Face model storage URI | Model Registry & Auth | huggingface, storage-uri, kserve | [12](chapters/12-kserve-llm-serving/README.md) | KServe `storageUri`에서 Hugging Face Hub model repository를 가리키는 URI prefix. 예: `hf://Qwen/Qwen2.5-0.5B-Instruct` |
| Storage Initializer | 모델 파일 준비 init 단계 | Serving Platform | kserve, model-storage, initializer | [12](chapters/12-kserve-llm-serving/README.md) | model artifact를 predictor가 실행되기 전에 지정된 storage URI에서 받아 `/mnt/models` 같은 위치에 준비하는 KServe 구성 요소 또는 초기화 흐름 |
| Custom Runtime | 사용자 정의 서빙 런타임 | Serving Runtime | kserve, custom-runtime, vllm | [12](chapters/12-kserve-llm-serving/README.md) | KServe 기본 runtime 대신 사용자가 직접 image, args, resource, supported model format을 정의한 runtime |
| Host Header Routing | Host header 기반 라우팅 | API & Protocol | gateway, host-header, kserve | [12](chapters/12-kserve-llm-serving/README.md) | gateway가 HTTP `Host` header를 보고 어떤 `InferenceService`로 요청을 보낼지 결정하는 방식. local port-forward 실습에서 자주 사용한다. |
| OpenAI Route Prefix | KServe OpenAI-compatible 경로 prefix | API & Protocol | openai-compatible, kserve, route | [12](chapters/12-kserve-llm-serving/README.md) | KServe LLM runtime이 OpenAI-compatible API를 노출할 때 사용하는 `/openai/v1` 경로 prefix |
| AWQ | Activation-aware Weight Quantization | LLM Internals | quantization, awq, memory | [13](chapters/13-advanced-serving-topics/README.md) | activation 분포를 고려해 중요한 weight 정보를 보존하려는 weight quantization 방식. GPU memory 절감 목적으로 자주 검토한다. |
| GPTQ | GPT Quantization | LLM Internals | quantization, gptq, model-format | [13](chapters/13-advanced-serving-topics/README.md) | 학습이 끝난 모델을 후처리로 quantize하는 방식 계열. 공개된 GPTQ model repo를 serving할 때 loader와 hardware 지원을 확인해야 한다. |
| bitsandbytes | bitsandbytes quantization/load library | LLM Internals | quantization, bnb, 4bit, 8bit | [13](chapters/13-advanced-serving-topics/README.md) | 8-bit/4-bit model loading과 fine-tuning 생태계에서 자주 쓰이는 library. serving 성능은 engine/kernel 지원에 따라 달라진다. |
| LoRA | Low-Rank Adaptation | LLM Internals | lora, adapter, finetuning | [13](chapters/13-advanced-serving-topics/README.md) | base model 전체를 바꾸지 않고 작은 adapter weight를 붙여 특정 task에 맞게 동작을 조정하는 방법 |
| Multi-LoRA Serving | 여러 LoRA adapter 동시 서빙 | Serving Runtime | lora, adapter, multi-tenant | [13](chapters/13-advanced-serving-topics/README.md) | 하나의 base model server에서 여러 adapter를 model 이름처럼 선택해 요청별로 serving하는 방식 |
| Speculative Decoding | 추측 디코딩 | Serving Engine | speculative-decoding, draft-model, latency | [13](chapters/13-advanced-serving-topics/README.md) | 작은 draft/proposer가 후보 token을 먼저 만들고 target model이 검증해 decode latency를 줄이려는 기법 |
| Draft Model | 초안 모델 | Serving Engine | speculative-decoding, draft-model | [13](chapters/13-advanced-serving-topics/README.md) | speculative decoding에서 후보 token을 빠르게 제안하는 작은 모델 또는 보조 모델 |
| Response Cache | 응답 캐시 | Operations | cache, response, tenant | [13](chapters/13-advanced-serving-topics/README.md) | 같은 요청에 대해 이미 생성된 최종 응답을 저장했다가 재사용하는 방식. tenant와 권한이 섞이면 위험하다. |
| Rate Limiting | 요청 속도 제한 | Operations | rate-limit, quota, gateway | [13](chapters/13-advanced-serving-topics/README.md) | 사용자, IP, API key, tenant 같은 기준으로 요청 속도나 횟수를 제한해 server와 다른 사용자를 보호하는 방법 |
| API Key | API 키 | Model Registry & Auth | auth, api-key, secret | [13](chapters/13-advanced-serving-topics/README.md) | API 호출자를 식별하거나 접근을 제한하기 위한 secret 문자열. Git에 저장하지 않고 회전/폐기 전략을 둔다. |
| Multi-Tenant Serving | 다중 테넌트 서빙 | Operations | multi-tenant, quota, isolation | [13](chapters/13-advanced-serving-topics/README.md) | 여러 사용자/팀/고객이 같은 serving system을 공유하는 구조. 인증, 권한, quota, cache, 관측을 tenant 기준으로 분리해야 한다. |
